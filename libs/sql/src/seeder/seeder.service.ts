...
export class SeederService {
  ...
  private async cleanExistingData(seed: Seed<any>): Promise<boolean> {
    try {
      ...
      await this.sequelize.models[seed.model].destroy({
        where: {},
        truncate: seed.alwaysRule === 'truncate',
        force: seed.alwaysRule === 'truncate',
      });
      return true;
    } catch (error) {
      this.logger.error(
  ...
  private async seedData(seed: Seed<any>) {
    ...
    for (let index = 0; index < seed.data.length; index++) {
      ...
      if (
        ...
        try {
          const [record, created] = await this.sequelize.models[
            seed.model
          ].findOrCreate({
          ...
          if (!created && seed.alwaysRule === 'update') {
